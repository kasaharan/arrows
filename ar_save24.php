<?php
	...
	/* ハッシュチェック */
	check_hash(post('dummy').post('sendtime').my_urlencode($name).$pwd.$scenarioIdx.$exp.$magatama.$noroi.$clearframecnt.$win.$personcnt.$dmgrate.$addexp.$ch.$daily);
	...
	if ($magatama == 1)
	{
		...
		if (($scenarioNo == 44) || ($scenarioNo == 46) || ($scenarioNo == 48) || ($scenarioNo == 50))
		{
			//報告書用
			$type = mt_rand(0, 2);
		}
