<?php
	...
	else if ($type == 's')
	{
		...
		//新しいのがリストの上に来るよう更新
		$stmt = $pdo->prepare('
		UPDATE ar_friend SET date = NOW() WHERE name = :friend AND friend = :name
		');
		$stmt->bindValue(':name', $name);
		$stmt->bindValue(':friend', $friend);
		$stmt->execute();
	...
	$stmt = $pdo->prepare("
	SELECT
		sort, flag, friend, date, login
	FROM (
		SELECT
			0 AS sort, 1 AS flag, F1.name AS friend, F1.date, '' AS login
		...
		SELECT
			1 AS sort, 0 AS flag, F1.friend, F1.date, DATE_FORMAT(U.login, '%Y/%m/%d %H:%i:%S') AS login
		FROM ar_friend AS F1
		JOIN ar_friend AS F2
			ON (F1.name = F2.friend AND F1.friend = F2.name)
		LEFT JOIN ar_user AS U
			ON (F1.friend = U.name)
		WHERE F1.name = :name
		...
		SELECT
			2 AS sort, 2 AS flag, F1.friend, F1.date, '' AS login
		FROM ar_friend AS F1
		LEFT JOIN ar_friend AS F2
			ON (F1.name = F2.friend AND F1.friend = F2.name)
		WHERE F1.name = :name
	...
	{
		$str .= $row['flag'] . ',' . my_urlencodeE($row['friend']) . ',' . my_urlencodeE($row['date']) . ',' . $row['login'] . '|';
	}
	print '&room=' . $str;
