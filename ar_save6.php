<?php
	...
	if (98 < $sac)
	{
		log_write_db('CHECK YAKAZU:' . $sac);
		//$addexp = mt_rand(-200, 0);	//まだ泳がす
		$dmgrate = 0;
	}
	//--------
	...
	//ユーザデータ取得(ログイン用)
	$stmt = $pdo->prepare('
	SELECT name, scenario, exp, release_cnt, itemno, DATE_FORMAT(login, "%Y%m%d%H%i%S") AS linfo FROM ar_user WHERE name = :name
	');
	$stmt->bindValue(':name', $name, PDO::PARAM_STR);
	$stmt->execute();
	$result = $stmt->fetchAll(PDO::FETCH_NAMED);
