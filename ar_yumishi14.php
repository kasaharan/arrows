<?php
	...
	if ($type == 'data')
	{
		...
		WHERE id IN ('HP', 'EXP5', 'EXP10', 'EXP15', 'EXP20', 'EXP25', 'DOCHO', 'NOROI', 'TADAN2', 'TADAN3', 'SELECT', 'LIGHT', 'YUMI2', 'WAZATAME', 'AUTO', 'AUTO1', 'NOROI2', 'YUMI3')
		OR id = (SELECT DISTINCT 'ITTEN'  FROM ar_useritem WHERE name = :name AND kind = 7)
		OR id = (SELECT DISTINCT 'HANABI' FROM ar_useritem WHERE name = :name AND kind = 9)
		OR id = (SELECT DISTINCT 'BABABA' FROM ar_useritem WHERE name = :name AND kind = 8)
		OR id = (SELECT DISTINCT 'REN'    FROM ar_useritem WHERE name = :name AND kind = 12)
		OR id = (SELECT DISTINCT 'SOU'    FROM ar_useritem WHERE name = :name AND kind = 17)
		ORDER BY sort
		");
